...
RED.subflow = (function() {
    ...
    function addSubflowInput(id) {
        ...
        RED.view.redraw();
        $("#red-ui-subflow-input .spinner-value").text(subflow.in.length);
        RED.events.emit("subflows:change",subflow);
    }

    function removeSubflowInput(removedSubflowInputs) {
        console.warn("removeSubflowInput 0");
        var activeSubflow = RED.nodes.subflow(RED.workspaces.active());
        if (activeSubflow.in.length === 0) {
            console.warn("removeSubflowInput 1");
            return;
        }
        if (typeof removedSubflowInputs === "undefined") {
            console.warn("removeSubflowInput 2");
        ...
        for (i=0;i<removedSubflowInputs.length;i++) {
            console.warn("removeSubflowInput 3 : %s", removedSubflowInputs.length);
            var input = removedSubflowInputs[i];
            activeSubflow.in.splice(input.i,1);
            var subflowRemovedLinks = [];
            var subflowMovedLinks = [];
            RED.nodes.eachLink(function(l) {
                console.warn("removeSubflowInput 4 : l.target.type : %s", l.target.type);
                console.warn("removeSubflowInput 4 : l.source.z : %s", l.source.z);
                console.warn("removeSubflowInput 4 : l.source.i : %s", l.source.i);
                console.warn("removeSubflowInput 4 : input.i : %s", input.i);
                console.warn("removeSubflowInput 4 : activeSubflow.id : %s", activeSubflow.id);
                console.warn("removeSubflowInput 4 : l.source.type : %s",  l.source.type);

                if (l.source.z == activeSubflow.id && l.source.i == input.i) {
                    console.warn("removeSubflowInput 5");
                    subflowRemovedLinks.push(l);
                }
                if (l.source.type == "subflow:"+activeSubflow.id) {
                    console.warn("removeSubflowInput 6");
                    if (l.sourcePort == input.i) {
                        console.warn("removeSubflowInput 7");
                        subflowRemovedLinks.push(l);
                    } else if (l.sourcePort > input.i) {
                        console.warn("removeSubflowInput 8");
                        subflowMovedLinks.push(l);
                    }
                }
            });

            console.warn("removeSubflowInput 9");
            ...
            for (var j=input.i;j<activeSubflow.in.length;j++) {
                console.warn("removeSubflowInput 10");
