...
RED.view = (function() {
    ...
    function showQuickAddDialog(point, spliceLink, targetGroup, touchTrigger) {
        ...
        var rebuildQuickAddLink = function() {
            ...
            var numInputs = (quickAddLink.portType === PORT_TYPE_INPUT)?(quickAddLink.node.inputs || 1):1;
            var portY = -((numOutputs-1)/2)*13 + ((numInputs-1)/2)*13;
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode === RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                ...
                var numInputs = (drag_line.portType === PORT_TYPE_INPUT)?(drag_line.node.inputs || 1):1;
                var portY = -((numOutputs-1)/2)*13 + ((numInputs-1)/2)*13;
    ...
    function deleteSelection() {
        if (RED.view.DEBUG) { console.warn("deleteSelection"); }
        if (mouse_mode === RED.state.SELECTING_NODE) {
            return;
        }
        if (portLabelHover) {
        ...
        if (workspaceSelection.length > 0) {
            ...
            if (movingSet.length() > 0) {
                ...
                if (removedSubflowOutputs.length > 0) {
                    ...
                    console.warn("result 1 : %s", result);
                    if (result) {
                        removedLinks = removedLinks.concat(result.links);
                    }
                }

                if (removedSubflowInputs.length > 0) {
                    result = RED.subflow.removeInput(removedSubflowInputs);
                    console.warn("result 2 : %s", result);
                    if (result) {
                        removedLinks = removedLinks.concat(result.links);
                    }
                }
    ...
    function portMouseDown(d,portType,portIndex, evt) {
        if (RED.view.DEBUG) { console.warn("portMouseDownX", mouse_mode,d,portType,portIndex); }
        evt = evt || d3.event;
        if (evt === 1) {
            return;
        }
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            link.each(function(d) {
                ...
                if (d.added || d===selected_link || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    ...
                    var numInputs = d.source.inputs || 1;
                    var y = -((numOutputs-1)/2)*13 + 13*((numInputs-1)/2);
